...
export function Orders() {
  ...
  async function OrderUpdate(id: string){
    ...
    try {
      ...
      Alert.alert("Chmado", "Chamado atualizado!");
      fetchOrders();
      
    } catch {
      Alert.alert("Chamados", "Não foi possível atualizar os chamados");
    }
  }

  function handleOrderUpadate(id: string) {
    Alert.alert(
    "Chamado",
    "Encerrar chamado?",
    [
...

      {
        isLoading ?
          ...
          : <FlatList
            ...
            renderItem={({ item }) => (
              <Order data={item} onPress={() => handleOrderUpadate(item._id)} />
            )}
